...
int main(int argc, char** argv) {
    args::ArgumentParser parser("Mesh Stitching - Stitch open mesh borders");

    args::Positional<std::string> input_file(parser, "input", "Input surface mesh file");
    args::Positional<std::string> output_file(parser, "output", "Output stitched mesh file");
    ...
    std::cout << "mesh loaded: " << original_vertices << " vertices, " << original_faces << " faces" << std::endl;

    int border_edges = 0;
    for (auto e : mesh->edges()) {
        if (mesh->is_border(e)) border_edges++;
    }
    std::cout << "mesh has " << border_edges << " border edges" << std::endl;

    std::cout << "stitching borders..." << std::endl;
    SurfaceMeshStitching stitcher(mesh.get());
    stitcher.apply();

    std::cout << "border stitching complete" << std::endl;
    ...
    if (output_file) {
        ...
        out_path = file_path.substr(0, file_path.find_last_of('.')) + "_stitched.ply";
    }
