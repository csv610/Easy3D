...
int main(int argc, char** argv) {
    args::ArgumentParser parser("Mesh Parameterization");

    args::Positional<std::string> input_file(parser, "input", "Input surface mesh file");
    args::Positional<std::string> output_file(parser, "output", "Output mesh file with UV coordinates");
    args::ValueFlag<std::string> method(parser, "method", "Method: lscm, harmonic (default: lscm)", {'m', "method"}, "lscm");
    ...
    std::cout << "parameterizing (method=" << method_name << ")..." << std::endl;

    SurfaceMeshParameterization para(mesh.get());
    ...
    std::cout << "parameterization complete" << std::endl;
    std::cout << "UV coordinates stored in vertex property 'v:texcoord'" << std::endl;

    std::string out_path;
    if (output_file) {
        out_path = args::get(output_file);
    } else {
        out_path = file_path.substr(0, file_path.find_last_of('.')) + "_parameterized.ply";
    }
