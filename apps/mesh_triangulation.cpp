...
int main(int argc, char** argv) {
    args::ArgumentParser parser("Mesh Triangulation");

    args::Positional<std::string> input_file(parser, "input", "Input surface mesh file");
    args::Positional<std::string> output_file(parser, "output", "Output triangulated mesh file");
    ...
    SurfaceMeshTriangulation triangulator(mesh.get());
    triangulator.triangulate();

    std::cout << "triangulated: " << mesh->n_vertices() << " vertices, " << mesh->n_faces() << " faces" << std::endl;
    std::cout << "all faces are now triangles" << std::endl;

    std::string out_path;
    if (output_file) {
        out_path = args::get(output_file);
    } else {
        out_path = file_path.substr(0, file_path.find_last_of('.')) + "_triangulated.obj";
    }
